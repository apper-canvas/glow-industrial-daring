...
const ServiceCard = ({ service, className = "" }) => {
  const parseCapabilities = (capString) => {
    try {
      if (!capString) return [];
      return typeof capString === 'string' ? JSON.parse(capString) : capString;
    } catch (e) {
      return capString?.split('\n').filter(Boolean) || [];
    }
  };

  const capabilities = parseCapabilities(service.capabilities_c);
  ...
  return (
    <Card hover className={`service-card p-6 h-full ${className}`}>
      <div className="flex items-center justify-center w-16 h-16 bg-gradient-to-br from-primary/10 to-accent/10 rounded-xl mb-6">
        <ApperIcon name={service.icon_c || "Cog"} className="w-8 h-8 text-primary" />
      </div>
      <h3 className="text-xl font-bold text-gray-900 mb-3">{service.name_c}</h3>
      <p className="text-gray-600 mb-4 flex-grow">{service.description_c}</p>
